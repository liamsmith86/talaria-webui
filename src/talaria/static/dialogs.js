...
import { state, update, connect, toast, fail, refreshSessions, newConversation, openSession } from './store.js';
...
export function Dialog({ title, children, onClose, wide = false }) {
  ...
  return html`<dialog class=${`dialog ${wide ? 'wide' : ''}`} ref=${ref} onCancel=${e => { e.preventDefault(); onClose(); }} onClick=${e => { if (e.target === e.currentTarget) { const r = e.currentTarget.getBoundingClientRect(); if (e.clientX < r.left || e.clientX > r.right || e.clientY < r.top || e.clientY > r.bottom) onClose(); } }}>
    <div class="dialog-heading"><h2>${title}</h2><${IconButton} name="close" label="Close dialog" onClick=${onClose}/></div>${children}
  </dialog>`;
}
...
export function Connection({ initial = false, onClose }) {
  ...
  useEffect(() => { api('/connection').then(d => { setUrl(d.url); setKeySet(d.key_set); }).catch(e => setError(e.message)); }, []);
  async function submit(save) {
    setBusy(true); setError(''); setTested(false);
    try {
      await api(save ? '/connection' : '/connection/test', { method: save ? 'PUT' : 'POST', body: { url, api_key: key } });
      if (save) { await connect(); onClose(); toast('Connected to Hermes'); }
      else setTested(true);
  ...
  return html`<${Dialog} title=${initial ? 'Meet your Hermes' : 'Connection'} onClose=${onClose}>
    ...
    <form onSubmit=${e => { e.preventDefault(); submit(true); }}>
      <label class="field">Hermes address<input type="url" value=${url} onInput=${e => { setUrl(e.target.value); setTested(false); }} required spellcheck="false" placeholder="http://127.0.0.1:8642"/></label>
      <label class="field">API key<input type="password" value=${key} onInput=${e => { setKey(e.target.value); setTested(false); }} placeholder=${keySet ? 'Saved securely · leave blank to keep' : 'Your Hermes API server key'} autocomplete="new-password"/></label>
...
export function Settings({ onClose }) {
  ...
  function appearance(key, value) {
    writeStorage(key, value); document.documentElement.dataset[key] = value;
  ...
  return html`<${Dialog} title="Make yourself at home" onClose=${onClose}>
    ...
    <section class="setting-section"><h3>Appearance</h3><div class="appearance-options" role="group" aria-label="Appearance">
      ${[['system', 'monitor', 'System'], ['light', 'sun', 'Light'], ['dark', 'moon', 'Dark']].map(([id, icon, label]) => html`<button class=${`appearance-option ${theme === id ? 'selected' : ''}`} aria-pressed=${theme === id} onClick=${() => appearance('theme', id)}><${Icon} name=${icon}/>${label}</button>`)}
    </div></section>
    <section class="setting-section"><h3>Accent color</h3><div class="palette-options" role="group" aria-label="Accent color">
      ${[['blue', 'Blue'], ['sage', 'Sage'], ['violet', 'Violet'], ['rose', 'Rose']].map(([id, label]) => html`<button class=${`palette-option ${palette === id ? 'selected' : ''}`} aria-pressed=${palette === id} onClick=${() => appearance('palette', id)}><span class=${`swatch ${id}`}>${palette === id && html`<${Icon} name="check" size=${16}/>`}</span>${label}</button>`)}
    </div></section>
    <section class="setting-section setting-links"><button onClick=${() => update({ modal: 'connection' })}><span><${Icon} name="link"/>Hermes connection</span><${Icon} name="chevron"/></button>
      <button onClick=${async () => { try { await api('/logout', { method: 'POST', body: {} }); location.reload(); } catch (e) { fail(e); } }}><span><${Icon} name="logout"/>Sign out</span></button>
  ...
  async function submit(e) {
    ...
    try {
      ...
      const result = await api(mode === 'fork' ? `${base}/fork` : base, {
        method: mode === 'delete' ? 'DELETE' : mode === 'fork' ? 'POST' : 'PATCH',
        body: mode === 'delete' ? {} : { title: mode === 'fork' ? `${title} · branch`.slice(0, 160) : title },
      });
      if (mode === 'delete' && state.active === session.id) newConversation();
      await refreshSessions(); onClose();
      if (mode === 'fork') await openSession(result.id || result.session_id || result.session?.id);
      toast(mode === 'delete' ? 'Conversation deleted' : mode === 'fork' ? 'Conversation branched' : 'Conversation renamed');
    } catch (e) { setError(e.message); } finally { setBusy(false); }
  }
...
export function ModelPicker({ models, selected, onSelect, onClose }) {
  ...
  const filtered = models.filter(m => `${m.label} ${m.providerLabel} ${m.id}`.toLowerCase().includes(query.toLowerCase()));
  return html`<${Dialog} title="Choose a model" onClose=${onClose}>
    ...
    <div class="model-list"><button class="model-option" onClick=${() => { onSelect(null); onClose(); }}><span>Hermes default<small>Use your agent’s configured model</small></span>${!selected && html`<${Icon} name="check" size=${18}/>`}</button>
    ${filtered.map(m => html`<button class="model-option" onClick=${() => { onSelect(m); onClose(); }}><span>${m.label}<small>${m.providerLabel}</small></span>${selected?.id === m.id && selected?.provider === m.provider && html`<${Icon} name="check" size=${18}/>`}</button>`)}
